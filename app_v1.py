...
def get_subnode(stn,t_f):
    p,k = list(t_f.predecessors(stn)),list(test_fam.successors(stn))
#    l = 
    la = []
...
def make_subplot(test_node,o_net,net_dict,vis):
    ...
    #subnode_trace = make_nodes(o_net.subgraph(get_subnode(test_node,o_net)),vis)

    if vis == True:
        subnode_trace.hovertext=make_hovtext(l,net_dict)
...
def make_fig(pa,res):
    ...
    v,w = make_subplot(1,test_fam,res,False)
    ...
    fig.data[1].hovertext,fig.data[1].hoverinfo=make_hovtext(res,res),"text"
...
def load_output(n):
    if n:
        ...
        return html.Div(id='test-output',children=[
            #html.H5(children="Output loaded {n} times"),
            dcc.Graph(
                id='example-graph',
...
def update_point(selectInput,nfig):
    ...
    try:
        ...
        test_fam = generate_net(pd_gdula1,result)
        #print('debugging: ',selectInput['points'])
